...
def evaluate_zero_shot_busi_test(model, args, device, epoch=1, save_dir=None, visualize=False):
    """
    基于你当前 busi_eval_utils.py 的逻辑：
    - 图像：image_encoder(mask_ratio=0.0) 得到 global + 196 patch
    ...
    - 保存：busi_eval_history_local.csv（写到 save_dir）
    """
    model.eval()

    # 1) 数据加载（split_txt 改为 test.txt）
    ...
    test_dataset = BUSI_Dataset(
        ...
        split_txt=args.split_txt,  # ✅ test.txt
        transform=transform
    )
    ...
    for i, (imgs, labels) in enumerate(test_loader):
        ...
        for txt_input in text_inputs_list:
            ...
            txt_tokens = model.text_local_proj(txt_hidden[:, 1:, :])  # [B, L-1, proj_dim]
        ...
        all_scores.extend(probs[:, 1].cpu().tolist())  # malignant prob for AUC
...
def build_args():
    ...
    # ---- paths (single checkpoint mode) ----
    parser.add_argument(
        "--checkpoint",
        default=None,
        type=str,
        help="Single checkpoint path (if provided, batch mode is ignored)"
    )

    # ---- batch mode arguments ----
    parser.add_argument(
        "--checkpoint_dir",
        default="/media/profz/data1/hmd/MTG_newdata/v1",
        type=str,
        help="Directory containing checkpoints (for batch mode)"
    )
    parser.add_argument(
        "--checkpoint_prefix",
        default="checkpoint-",
        type=str,
        help="Checkpoint filename prefix (e.g., 'checkpoint-')"
    )
    parser.add_argument(
        "--checkpoint_suffix",
        default=".pth",
        type=str,
        help="Checkpoint filename suffix (e.g., '.pth')"
    )
    parser.add_argument(
        "--start_idx",
        default=0,
        type=int,
        help="Start checkpoint index (inclusive)"
    )
    parser.add_argument(
        "--end_idx",
        default=10,
        type=int,
        help="End checkpoint index (inclusive)"
    )

    # ---- output / others ----
    parser.add_argument(
        "--output_dir",
        default="/media/profz/data1/hmd/MTG_new/v2_2/mask_0.85",
        type=str
    )
    parser.add_argument(
        "--bert_path",
...
def main():
    ...
    if args.checkpoint is not None:
        ...
        epoch_tags = [args.epoch_tag]   # use provided tag
    else:
        # Batch mode: generate file list from start_idx to end_idx
        checkpoint_paths = []
        epoch_tags = []
        for idx in range(args.start_idx, args.end_idx + 1):
            path = os.path.join(args.checkpoint_dir, f"{args.checkpoint_prefix}{idx}{args.checkpoint_suffix}")
            if os.path.exists(path):
                checkpoint_paths.append(path)
                epoch_tags.append(idx)   # use index as epoch tag for logging
            else:
                print(f"[Warning] Checkpoint not found, skipping: {path}")

        if not checkpoint_paths:
            print("[Error] No valid checkpoints found in the specified range.")
            return

    # Loop over each checkpoint
    for ckpt_path, epoch_tag in zip(checkpoint_paths, epoch_tags):
        print("\n" + "="*60)
        print(f"Evaluating checkpoint: {ckpt_path}")
        print("="*60)

        # Build model (fresh instance for each checkpoint)
        ...
        evaluate_zero_shot_busi_test(
            ...
            args=args,
            device=device,
            epoch=epoch_tag,            # use epoch_tag to distinguish in CSV
